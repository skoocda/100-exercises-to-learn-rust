    ...
    T: Display + Send + Sync + 'static,
{
    ...
    let handle1 = tokio::spawn(fixed_reply_writer(first, Arc::clone(&reply)));
    let handle2 = tokio::spawn(fixed_reply_writer(second, reply));
    tokio::join!(handle1, handle2,);
}

async fn fixed_reply_writer<T>( listener: TcpListener, reply: Arc<T>) 
where
    T: Display + Send + Sync + 'static,
{
    loop {
